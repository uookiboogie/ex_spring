...
<script src="./resources/asset/js/json.min.js" ></script>
<script>
$(function(){
	userList();
...
function userSelect() {
	...
	$('body').on('click','#btnSelect',function(){
		...
		//var userId = $(this).closest('tr').data("userid"); //찾는 태그 여러 가지 방법
		//특정 사용자 조회
		$.ajax({
			url:'users/'+userId,
			type:'GET',
...
function userListResult(data) {
	...
	$.each(data,function(idx,item){ // for( i=0; i<data.length; i++) { item = data[i] }
		$('<tr data-userid="'+item.id+'">') //data 속성 이용법
